...
import {
  ...
  getRunnerConfig,
  type RunnerConfigData,
  type RunnerConfigResolveInput,
} from "./RunnerConfig";

const resolveRunnerConfig = (
  options: RunnerConfigResolveInput,
): Effect.Effect<
  RunnerConfigData,
...
describe("RunnerConfig", () => {
  ...
  it.effect(
    "fails when a recognized CLI option is missing a required separate value",
    () =>
      Effect.gen(function* () {
  ...
  it.effect(
    "fails when a recognized CLI option is provided with an empty inline value",
    () =>
      Effect.gen(function* () {
        const error = yield* Effect.flip(
          resolveRunnerConfig({
  ...
  it.effect("maps schema decoding failures to InvalidRunnerConfigError", () =>
    Effect.gen(function* () {
      const error = yield* Effect.flip(
        resolveRunnerConfig({
          argv: [],
