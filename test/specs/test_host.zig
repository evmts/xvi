...
pub const TestHost = struct {
    ...
    fn setBalanceVTable(ptr: *anyopaque, address: Address, balance: u256) void {
        ...
        self.balances.put(address, balance) catch |err| {
            std.debug.panic("TestHost.setBalance failed: {any}", .{err});
        };
    }
    ...
    fn setCodeVTable(ptr: *anyopaque, address: Address, code: []const u8) void {
        ...
        const owned_code = self.allocator.dupe(u8, code) catch |err| {
            std.debug.panic("TestHost.setCode dupe failed: {any}", .{err});
        };
        self.code.put(address, owned_code) catch |err| {
            self.allocator.free(owned_code);
            std.debug.panic("TestHost.setCode put failed: {any}", .{err});
        };
    }
    ...
    fn setStorage(ptr: *anyopaque, address: Address, slot: u256, value: u256) void {
        ...
        if (value == 0) {
            ...
            self.storage.put(key, value) catch |err| {
                std.debug.panic("TestHost.setStorage failed: {any}", .{err});
            };
        }
    }
    ...
    fn setNonceVTable(ptr: *anyopaque, address: Address, nonce: u64) void {
        const self: *Self = @ptrCast(@alignCast(ptr));
        self.setNonce(address, nonce) catch |err| {
            std.debug.panic("TestHost.setNonce failed: {any}", .{err});
        };
    }
};
